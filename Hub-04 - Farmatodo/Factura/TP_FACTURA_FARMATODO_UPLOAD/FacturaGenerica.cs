...
namespace TP_FacturaFarmatodo
{
	...
	public class ClaseFacturaFarmatodo
	{
		...
		public void InsertFacturaFarmatodo(string strDirectorioEntrada)
		{
			...
			string strNombreArchivoSalida = "";
			string Nombre="";
            int intPosDir = 0;

			DirectoryInfo DIR = new DirectoryInfo(Class1.strDirectorioEntrada);
			...
			try
			{
				...
				string[] Files = Directory.GetFiles(@strDirectorioEntrada, "*.txt");
				...
				{  
					...
					FileInfo miFileInfo  = new FileInfo(File);

					strNombreArchivo = File;

                    intPosDir = Int32.Parse(Class1.PosDir);

                    Nombre = File.Substring(intPosDir).Replace(".txt", "").Trim();

					// CREA UNA INSTANCIA DE StreamReader PARA LEER DEL ARCHIVO.
					using (StreamReader sr = new StreamReader(strNombreArchivo, System.Text.Encoding.Default)) 
					{
						...
						while ((strLineaLeida = sr.ReadLine()) != null) 
						{
							...
							else if(arrCamposLinea.GetValue(0).ToString() == "02")
							{
								...
								if(strTemporal=="NO")
								{
									...
									try
									{
										strNumeroDoc = arrCamposLinea.GetValue(1).ToString().Replace("'", "").Trim();
									...
									try
									{
										...
										strCodigoProveedorHub = arrCamposLinea.GetValue(19).ToString().Replace("'", "").Trim();
						...
						if((strEnvioCorreo=="SI"))
						{
							...
							if (strEmail!="" && strEmailCuerpo!="")
							{
								...
								{
									Class1.strCodError = ex1.Message;
                                    objTextFile.EscribirLog("  ---> ERROR -- ERROR ENVIANDO EL EMAIL ERRORES DE CARGA FACTURA ELECTRONICA.  CODIGO: " + Class1.strCodError + " correos: " + strEmail);								
								}
							}
						}
					...
					//MANDO A MOVER CADA UNO DE LOS ARCHIVOS LEIDOS A UNA CARPETA TEMPORAL
					string strDia = Convert.ToString(System.DateTime.Now.Day);
					string strMes = Convert.ToString(System.DateTime.Now.Month);
					string strAnio = Convert.ToString(System.DateTime.Now.Year);
					string strHH = Convert.ToString(System.DateTime.Now.Hour);
					string strMM = Convert.ToString(System.DateTime.Now.Minute);
					string strSS = Convert.ToString(System.DateTime.Now.Second);

					string strFechaHoy = strDia+strMes+strAnio+strHH+strMM+strSS;

					strNombreArchivoEntrada= strNombreArchivo;
					strNombreArchivoSalida= Class1.strDirectorioSalida + Nombre + "_" + strFechaHoy + ".txt";

					try
					{

						System.IO.Directory.Move(strNombreArchivo, strNombreArchivoSalida);
					...
					{
						Class1.strCodError = ex.Message;								
						objTextFile.EscribirLog("  ---> ERROR -- ERROR MOVIENDO EL ARCHIVO:  " + strNombreArchivo + " A CARPETA NO CONFIGURADO ." + Class1.strCodError);
