...
namespace TP_FacturaFarmatodo
{
	...
	class Class1
	{
		...
		public static string strDirectorioEntrada= String.Empty;
		public static string strDirectorioSalida = String.Empty;
		//correo
        ...
        public static string PosDir = String.Empty;
		#endregion
		...
		static void Main()
		{
			...
			if (strDirectorioEntrada != "")
			{
					...
					try
					{
						...
						objFactura.InsertFacturaFarmatodo(strDirectorioEntrada);
					...
					{
						strCodError = ex.Message;
						objTextFile.EscribirLog("  ---> ERROR -- OCURRIO UN ERROR CON EL DIRECTORIO DE ENTRADA. CODIGO: " + strCodError);
					}
			...
			try
			{
				...
				while (reader.Read())
				{
					if(reader.AttributeCount > 0)
					{
						...
						if (reader.GetAttribute(0) == "EXP_DIR")
						{
							strDirectorioEntrada = reader.GetAttribute(1);
						}
						if (reader.GetAttribute(0) == "EXP_DIR_SAL")
						{
							strDirectorioSalida = reader.GetAttribute(1);
						}
                        ...
                        if (reader.GetAttribute(0) == "STREMAILCC")
                        {
                            strEmailCC = reader.GetAttribute(1);
                        }

                        if (reader.GetAttribute(0) == "STRSERVER")
                        {
                            strServer = reader.GetAttribute(1);
                        }

                        if (reader.GetAttribute(0) == "POSDIR")
                        {
                            PosDir = reader.GetAttribute(1);
                        }
