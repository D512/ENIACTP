					...
					while ((strLineaLeida = sr.ReadLine()) != null) 
					{
						...
						{	
							...
							if (strNumeroDoc != "")
							{
								if (strCodigoProveedorHub != "")
								{
									...
									query += " (NUMERO_DOC, FECHA_DOC, FECHA_GENERACION, CODIGO_COMPRADOR, NOMBRE_COMPRADOR, COD_PROV_HUB_TP, HUB_TP, STATUS_DOC, FECHA_STATUS, nombre_proveedor) ";
									query += " VALUES ";
									query += " ('" + arrCamposLinea.GetValue(1).ToString().Replace("'","") + "', CONVERT(DATETIME, '" + strFecha_Doc + "'), CONVERT(DATETIME, '" + strFecha_Generacion + "'), '" + arrCamposLinea.GetValue(6).ToString().Replace("'","") + "', 'Farmatodo S.A.', '" + strCodigoProveedorHub.ToString().Replace("'","") + "', 'Hub-04', '100', CONVERT(DATETIME, GetDate()), '" + strNombre_Proveedor.ToString().Replace("'","") + "') ";
									...
									try
									{
										...
										mySqlCommand = objBDatos.Comando(query, mySqlConnection);
										mySqlCommand.ExecuteNonQuery();
										objTextFile.EscribirLog("INSERTANDO ENCABEZADO DEL DOCUMENTO NUMERO: " + strNumeroDoc + " DEL PROVEEDOR: " + strCodigoProveedorHub +" Nombre:"+ strNombre_Proveedor);
						...
						else if(arrCamposLinea.GetValue(0).ToString() == "D")
						{
							...
							if (strErrorDetalles == "NO")
							{
								...
								query += " (NUMERO_DOC, COD_PROV_HUB_TP, CODIGO_PROD_COMPRADOR, CODIGO_PROD_BARRAS, CODIGO_PROD_PROVEEDOR, CANTIDAD_ORDENADA, CANTIDAD_COMPROMETIDA, CANTIDAD_MINIMA, CANTIDAD_MAXIMA, CANTIDAD_EXISTENCIA, CANTIDAD_REORDEN, CANTIDAD_TRANSITO, UNIDAD_MEDIDA, CODIGO_LOCALIZACION, VENTAS_DIARIA, DESCRIPCION, HUB_TP, VENTA_NETA, VENTA_PROMOCIONAL) ";
								query += " VALUES ";
								query += " ('" + arrCamposLinea.GetValue(1).ToString().Replace("'","") + "', '" + strCodigoProveedorHub.ToString().Replace("'","") + "', '" + arrCamposLinea.GetValue(3).ToString().Replace("'","") + "', '" + arrCamposLinea.GetValue(4).ToString().Replace("'","") + "', '" + arrCamposLinea.GetValue(5).ToString().Replace("'","") + "', " + arrCamposLinea.GetValue(6) + ", " + arrCamposLinea.GetValue(7) + ", " + arrCamposLinea.GetValue(8) + ", " + arrCamposLinea.GetValue(9) + ", " + arrCamposLinea.GetValue(10) + ", " + arrCamposLinea.GetValue(11) + ", ";
								query += " " + arrCamposLinea.GetValue(12) + ", '" + arrCamposLinea.GetValue(13).ToString().Replace("'","") + "', '" + arrCamposLinea.GetValue(14).ToString().Replace("'","") + "', " + arrCamposLinea.GetValue(15) + ", '" + arrCamposLinea.GetValue(16).ToString().Replace("'","").Replace(",",".") + "', 'Hub-04', " + arrCamposLinea.GetValue(17) + ", " + arrCamposLinea.GetValue(18) + ") ";
