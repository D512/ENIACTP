					...
					while ((strLineaLeida = sr.ReadLine()) != null) 
					{
						...
						{							
							...
							if ((strNumeroDoc == strTempNroDoc) && (strErrorDetalles == "NO"))
							{
								...
								objTextFile.EscribirLog("SE INSERTARON Y/O MODIFICARON " + intContDet + " REGISTROS DE DETALLE PARA EL DOCUMENTO NUMERO: " + strNumeroDoc);
							}
							...
							if (strNumeroDoc != "")
							{
								if (strCodigoProveedorHub != "")
								{
									...
									if (strCodigo_Despacho!="7591472900101" && strCodigo_Despacho!="")
									{
										...
										//BUSCAMOS EL NOMBRE DE DESPACHO Y LA DIRECCION EN LA TABLA FARMATODO LOCALIDADES
										query = "SELECT * FROM farmatodo_localidades where cod_ean = '" + arrCamposLinea.GetValue(11).ToString().Replace("'","").Trim() + "'";
										try
										{
											mySqlCommand=objBDatos.Comando(query,mySqlConnection);
											cursor=mySqlCommand.ExecuteReader();
											while(cursor.Read())
											{
												strNombre_Despacho = cursor["localizacion"].ToString().Trim();
												strDireccion_Despacho = cursor["direccion"].ToString().Trim().Replace("'","");
											}
											cursor.Close();
										}
										catch(Exception e)
										{
											Class1.objTextFile.EscribirLogError("  ---> ERROR REALIZANDO EL QUERY QUE VERIFICA LA EXISTENCIA DEL COD DESPACHO Y DIRECCION EN LA BD");
											Class1.objTextFile.EscribirLogError("     ---> ERROR :" + e.Message);
										}	

									}//FIN if (strCodigo_Despacho == "7591472900101") 
										...
										query += " TOTAL_VOLUMEN, ";
										query += " TOTAL_PESO, ";
										query += " MONTO_NETO,CONDICION_PAGO) ";
										query += " VALUES (";
										query += " '" + strHub_TP + "', ";
										...
										query += " '" + strNombre_Proveedor.Replace("'","").Trim() + "', ";
										...
										query += " '" + arrCamposLinea.GetValue(19).ToString().Trim() + "') ";
										...
										catch (Exception ex) 
										{
											...
											if((Class1.strCodError.Substring(0,24) == "Violation of PRIMARY KEY")||(Class1.strCodError.Substring(0,41) == "Cannot insert duplicate key row in object"))
											{
												...
												try
												{
													...
													query += " NOMBRE_PROVEEDOR = '" + strNombre_Proveedor.Replace("'","").Trim() + "', ";
													...
													query += "CONDICION_PAGO = '" + arrCamposLinea.GetValue(19).ToString().Trim() + "' ";
										...
										catch (Exception ex) 
										{
											...
											if((Class1.strCodError.Substring(0,24) == "Violation of PRIMARY KEY")||(Class1.strCodError.Substring(0,41) == "Cannot insert duplicate key row in object"))
											{
												...
												// HACER EL UPDATE DEL ENCABEZADO YA QUE EL REGISTRO EXISTE EN LA BASE DE DATOS
													...
													if(arrCamposLinea.GetValue(11).ToString().Trim()!="")
														query += " COSTO_BASE_UNIDAD_REATIL = " + arrCamposLinea.GetValue(11).ToString().Trim()+ ", ";
													else
													...
													if(mySqlConnection.State.ToString() == "Closed")
													{
														...
														while ((intReintentosAplicados) == intReintentos || (strErrorIntentos != "SI"))											
														{
															...
															{
																Class1.strCodError = ex2.Message;								
																objTextFile.EscribirLog("  ---> ERROR -- ERROR ABRIENDO LA BASE DE DATOS, INTENTO NUMERO: " + intReintentosAplicados + "  CODIGO: " +  Class1.strCodError);																intReintentosAplicados += intReintentosAplicados;
											...
											{
												objTextFile.EscribirLogError("  ---> ERROR -- ERROR INSERTANDO EL REGISTRO DE DETALLE DEL DOCUMENTO NUMERO: " + strNumeroDoc + " DEL PROVEEDOR: " + strCodigoProveedorHub + "  CODIGO: " + Class1.strCodError+ " .LINEA:" + intNumeroLinea);
												objTextFile.EscribirLogError("  ---> QUERY:" + query);
